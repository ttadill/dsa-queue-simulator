...
class Simulation:
    def __init__(self):
        ...
        pygame.display.set_caption("Traffic Simulation – Queue Visual")
        self.clock = pygame.time.Clock()
        self.font = pygame.font.SysFont("Arial", 28)

        self.intersection = Intersection()  # keep your existing class
        self.roads = ["A", "B", "C", "D"]
        self.current_green = "A"
        self.green_timer = 0
        self.green_duration = 200  # frames (~3s)

        # Create VisualVehicles from Intersection lanes
        self.visual_vehicles = {r: [] for r in self.roads}
        self.sync_visuals_with_lanes()

    def sync_visuals_with_lanes(self):
        """
        Wrap vehicles in visual objects based on their lane queue.
        """
        for road in self.roads:
            lane_queue = self.intersection.lanes[road].queue  # assume list
    ...
    def run(self):
        ...
        while running:
            ...
            self.intersection.update()  # existing logic

            # Update visuals based on queue
            for road in self.roads:
                ...
                # Remove cars that left intersection (dequeue)
                self.visual_vehicles[road] = [
